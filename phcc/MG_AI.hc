...
void  (entity targ,float range_mod)get_visibility =  {
local float base = 0.00000;
local float divider = 0.00000;
local float attack_mod = 0.00000;
   ...
   if ( (targ.drawflags & MLS_ABSLIGHT) ) {
      ...
      if ( (targ.drawflags & MLS_POWERMODE) ) {
         ...
         base = (targ.light_level / 75.00000);
   ...
   if ( range_mod ) {
      ...
      range_mod = (vlen ( (targ.origin - self.origin)) / 333.00000);
   } else {

      range_mod = 1.00000;

   }
   if ( (targ.last_attack > (time - 3.00000)) ) {
...
local entity targ;
   ...
   if ( (targ.origin_z != self.absmin_z) ) {
      ...
      goaldist = (((targ.absmin_z + targ.absmax_z) * 0.50000) - ((self.absmax_z + self.absmin_z) * 0.50000));//shan??
...
local float horz_size = 0.00000;
   makevectors ( self.angles);
   checkdir = v_right;
   org = ((self.origin + self.proj_ofs) + (checkdir * self.size_x)); //shan??
...
local vector vofs = '0.00000 0.00000 0.00000';
   if ( !targ ) {
   ...
   makevectors ( self.angles);
   get_visibility ( targ, TRUE);
   ofs = (((1.00000 - targ.visibility) - (skill / 10.00000)) * 0.10000);
   if ( ((skill < 3.00000) && (ofs > 0.00000)) ) {

      vofs = (((v_up * 0.50000) * random((0.00000 - ofs),ofs)) + ((v_right * 1.50000) * random((0.00000 - ofs),ofs)));
      return ( vofs );
