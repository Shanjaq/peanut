...
local float damg = 0.00000;
   ...
   if ( (((other.health <= damg) && (other.thingtype == THINGTYPE_FLESH)) && (random() < 0.10000)) ) {
      ...
      if ( other.health ) {
		...
		if (random() > 0.5) // BAER added this to make it make less sounds
...
// BAER ALL NEW CODE!
void  (vector org,float set_speed,vector desty)do_blizzardshard =  {
local entity missile, destent;
local vector vec = '0.00000 0.00000 0.00000';
   missile = spawn ( );
   ...
   missile.nextthink = (time + HX_FRAME_TIME);
	...
	destent = spawn();
	setorigin(destent, desty);

	vec = (destent.origin - missile.origin);
	vec = normalize (vec);
