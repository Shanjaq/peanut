...
void  (entity loser,entity forwhom)SnowJob =  {
	...
	// loser.origin = (loser.origin + '0 0 7');  
	...
	if ( (loser.classname != "player") ) {
		...
		loser.freeze_time = (time + 15.00000); // BAER gave longer freeze times
		...
		loser.experience_value = 0.00000;
		oself = self;
		self = loser;
		SUB_UseTargets ( );
		self = oself;
...
void  ()FreezeTouch =  {
	...
	if ( (((other.takedamage && other.health) && (other.frozen <= 0.00000)) && (other.thingtype == THINGTYPE_FLESH)) ) {
		...
		if ( (other.frozen <= 0.00000) ) {
			...
			T_RadiusDamage ( self, self.owner, 40.00000, self.owner);
			self.touch = SUB_Null;
			remove(self.movechain);
			remove(self);
...
void  ()SnowballSparkle =  {
	if (self.cnt < 90) {
		self.cnt += 1;
		if (self.classname == "corona") {
...
void  ()FireFreeze =  {
	...
	self.attack_finished = (time + 0.16250);
	...
	corona.think = SnowballSparkle;
	AdvanceThinkTime(corona,HX_FRAME_TIME);
...
// BAER needed so it runs smooth, see?
void  (vector org,float set_speed,vector dest)do_blizzardshard;
...
void  ()blizzard_think =  {
	...
	while ( beam_count ) {
		...
		traceline ( dir, (dir + '0.00000 0.00000 1024.00000'), TRUE, self); // BAER changed from 128
		top = trace_endpos;
		traceline ( dir, (dir - '0.00000 0.00000 1024.00000'), TRUE, self); // BAER changed from 128
		bottom = trace_endpos;
		// BAER new code
		top_mod = top + RandomVector('20 20 50');
		//CreateWhiteSmoke (top_mod, '0 0 0', HX_FRAME_TIME);
		do_blizzardshard(top_mod, 400, bottom);
		/* BAER commented the streams out
...
void  ()make_blizzard =  {
	...
	self.solid = SOLID_BBOX; // BAER changed from SOLID_NOT.
