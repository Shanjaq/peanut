...
void  ()player_pain =  {
   if ( (((self.last_attack + 0.50000) > time) || self.button0) ) {
...
local vector org = '0.00000 0.00000 0.00000';
   if ( (self.decap == 2.00000) ) {
      ...
      if ( (((trace_fraction < 1.00000) && !(trace_ent.flags2 & FL_ALIVE)) && (trace_ent.solid == SOLID_BSP)) ) {
...
void  ()PlayerCrouch =  {
   ...
   if ( (self.hull == HULL_PLAYER) || (self.UFO == 1) ) {

      PlayerCrouching ( );
   } else {

      if ( (self.hull == HULL_CROUCH) || (self.UFO == 1) ) {
...
void  ()PlayerDie =  {
   if ( (self.viewentity != self) ) {
      ...
      CameraViewAngles ( self, self, 0, 0, 0);
...
void  ()player_behead =  {
   ...
   if ( !self.cnt ) {
bloodymess();
      MeatChunks ( (self.origin + '0.00000 0.00000 50.00000'), (v_up * 200.00000), 3.00000, self);
   } else {
