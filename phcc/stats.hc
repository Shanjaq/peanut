...
float mana_table [20]   = { 84.00000,
    ...
    2.00000,    96.00000,    106.00000,    10.00000,
...
local float wis_mod = 0.00000;
   ...
   wis_mod = (ToEnt.wisdom - 11.00000);
   Amount += ((Amount * wis_mod) / 20.00000);
...
local float pos = 0.00000;
   ...
   if ( ((loser.level - number) > 1.00000) ) {

      loser.level -= number;
      pos = ((loser.playerclass - 1.00000) * (MAX_LEVELS + 1.00000));
      loser.experience = (ExperienceValues->((pos + loser.level) - 2.00000));
   } else {
   ...
   if ( (loser.level <= 2.00000) ) {

      loser.flags ^= FL_SPECIAL_ABILITY1;

   }
   if ( (loser.level <= 5.00000) ) {
