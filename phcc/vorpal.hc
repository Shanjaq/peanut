...
float VORP_ADD_DAMAGE   =  10.00000;
...
void  ()vshock_run =  
{
	...
	if ( self.enemy.health ) 
	{
		...
		if (random() >= 0.5)
		{
			sound ( self, CHAN_WEAPON, "baer/gntact1.wav", 1.00000, ATTN_NORM);
		}
		/*	else
		{
			sound (self, CHAN_WEAPON, "baer/gntpow.wav", 1.00000, ATTN_NORM);
		}*/
		...
		{
			strikelength = random(128, 256);
		}
		zapangle = RandomVector ( '360.00000 360.00000 360.00000');
		makevectors ( zapangle);
		fromspot = (self.origin + (v_forward * 16.00000));
		tospot = (self.origin + (v_forward * (random(32.00000) + strikelength)));
		do_lightning ( self, 24, STREAM_ATTACHED, 1.00000, fromspot, tospot, random(15,30));
	...
	if (self.lifetime < time)
	{
		shock_cnt = random(0.00000,7.00000);
		//shock_cnt = 1.00000; BAER commented line out to see.
...
void  (entity victim)vorp_shock =  {
	...
	newent.drawflags = MLS_ABSLIGHT;
	newent.abslight = 1; // BAER changed from 0.5. looks crappy in gl.
...
void  ()vorpmissile_run =  {
	...
	if (random() > 0.66)
	{
		sound ( self, CHAN_WEAPON, "baer/wepele2.wav", 1.00000, ATTN_NORM);
	}
	// BAER end of new code
};
...
void  ()vorpmissile_birth =  
{
	...
	if (self.frame >= 14.00000)  
	{
		...
		makevectors(self.angles);
		self.velocity = normalize(v_forward); 
		self.velocity = (self.velocity * 1000.00000);
		// BAER End new code
		vorpmissile_run();
	}
};
...
void  ()launch_vorpal_missile =  
{
	...
	//missile.velocity = normalize ( v_forward); BAER commented out to make it grow before launch
	//missile.velocity = (missile.velocity * 1000.00000);
	missile.frags = TRUE;
	missile.touch = vorpmissile_touch;
	//missile.angles = vectoangles ( missile.velocity); BAER commented out
	missile.angles = self.v_angle;
	//missile.angles_x += 180.00000; BAER was causing problems.
	//missile.drawflags = MLS_ABSLIGHT; BAER commented out
	//missile.abslight = 0.5000;
	missile.drawflags = MLS_POWERMODE;
	setorigin ( missile, (self.origin + (v_up * 40.00000)+ (v_forward * 16))); // BAER added v_forward bit
...
void  ()moveswipe =  {
	...
	org = ((self.owner.origin + self.owner.view_ofs + '0 0 4') - '0.00000 0.00000 25.00000'); // BAER added the '0 0 4' part
	setorigin ( self, org);
...
void  ()SpawnSwipe =  {
	...
	// BAER New line of code
	swipe.scale = 0.5;	// To cut back on the size of the bloody bugger
};
