...
void  ()BecomeImp =  {
	...
	tracearea ( self.origin, (self.origin + '0.00000 0.00000 1.00000'), self.mins, self.maxs, FALSE, newmis);
	while ( (((trace_fraction < 1.00000) || trace_allsolid) && (move_cnt < 36.00000)) ) {

		setorigin ( newmis, (newmis.origin + '0.00000 0.00000 1.00000'));
		tracearea ( newmis.origin, (newmis.origin + '0.00000 0.00000 1.00000'), self.mins, self.maxs, FALSE, newmis);
		move_cnt += 1.00000;
...
void  ()Use_Proximity_Mine =  {
	...
	newmis.dmg = (50.00000 + (self.level * 10.00000));
	if ( (newmis.dmg > 120.00000) ) {

		newmis.dmg = 120.00000;
...
//---------------------------------------
// Baer added new stuff to the equation.
// Most of this timebomb code is completely new.
//----------------------------------------
...
void  ()TimeBombBoom =  
{
	...
	while (counting > 0)
	{
		vel = RandomVector('20 20 20');
		velz = random(10,20);
		randomnumber = random();
		if (randomnumber > 0.66)
		starteffect ( CE_NEW_EXPLOSION, self.origin+vel, vel, HX_FRAME_TIME*random(1,3));
		else //if (randomnumber > 0.33)
		starteffect ( CE_SM_EXPLOSION, self.origin+vel, vel, HX_FRAME_TIME*random(1,3));
		//else
		//{
		//	vel_z += velz;
		//	starteffect ( CE_BONE_EXPLOSION, self.origin, vel, HX_FRAME_TIME);
		//}
		counting -= 1;
	}
...
void  ()TimeBombTouch =  
{
	if (!other.takedamage || other == self.owner) 
	{
		return ;
	}
	if (other)
	{
		other = self.enemy;
		T_Damage ( other, self, self.owner, 150.00000);
	}
...
void  ()Use_TimeBomb =  
{
	...
	//newmis.hull = HULL_IMPLICIT;
	//newmis.th_die = TimeBombTouch;
	...
	makevectors(self.v_angle);
	//newmis.velocity = normalize ( v_forward) * 70;
...
void  ()UseBlast =  
{
	...
	sound ( self, CHAN_WEAPON, "raven/blast.wav", 1.00000, ATTN_NORM); // BAER added so you know its being used
...
void  ()UseInventoryItem =  {
	if ( (self.health <= 0.00000) || (self.predebt == 1) ) {
			...
			if ( ((self.inventory == INV_HP_BOOST) && self.cnt_h_boost) ) {
				...
				if ( ((self.inventory == INV_SUPER_HP_BOOST) && self.cnt_sh_boost) ) {
					...
					if ( ((self.inventory == INV_MANA_BOOST) && self.cnt_mana_boost) ) {
						...
						// BAER added this glyph time stuff
						if ( ((self.inventory == INV_GLYPH) && self.cnt_glyph && self.glyph_time < time) ) {

							self.cnt_glyph -= 1.00000;
							self.glyph_time = time + 0.1;
