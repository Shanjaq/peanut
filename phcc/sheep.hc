...
void  ()sheep_graze_a =  {
   ...
   if ( (((self.frame == 8.00000) && (random() < 0.50000)) && !(self.spawnflags & 1.00000)) ) {
	...
	if (self.health < 35) {
		self.health += 1;
		sound (self, CHAN_AUTO, "bite4.wav", 1, ATTN_NORM);
	}
      }
...
void  ()monster_sheep_bite =  {
	//sprint(self.owner, "BARF9");
   makevectors ( self.angles);
   traceline ( (self.origin + self.view_ofs), ((self.origin + self.view_ofs) + (v_forward * 36.00000) + (v_up * 36.00000)), FALSE, self);
   if (( trace_ent.takedamage ) && (trace_ent.model != "models/sheep.mdl")) {

      sound ( self, CHAN_VOICE, "spider/bite.wav", 1.00000, ATTN_NORM);
      SpawnPuff ( trace_endpos, '0.00000 0.00000 0.00000', 3.00000, trace_ent);
	if (((other.thingtype == THINGTYPE_HAY) || (other.thingtype == THINGTYPE_LEAVES) || (other.thingtype == THINGTYPE_WOOD_LEAF)) && (self.health < self.max_health)) {
		self.health += 3;
	}
		if (self.spelldamage != 0.00000)
			T_Damage ( trace_ent, self, self, self.spelldamage);
		else
...
//   if ( (!(self.enemy.flags2 & FL_ALIVE) || !visible ( self.enemy)) ) {
   if ( !visible ( self.enemy) ) {
      ...
      if ( infront ( self.enemy) ) {
         ...
         if ( ((range ( self.enemy) == RANGE_MELEE) && (self.enemy != self.controller) && (self.attack_finished < time)) ) {
...
{
	if (loser.model == "models/sheep.mdl") // BAER added this to stop the sounds	
