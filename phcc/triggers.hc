...
void  ()counter_use =  {
   ...
   if ( (((activator.classname == "player") && ((self.spawnflags & SPAWNFLAG_NOMESSAGE) == 0.00000)) && !deathmatch) ) {
      ...
      sound ( activator, CHAN_ITEM, "misc/secret.wav", 1.00000, ATTN_NORM);
...
local float num_found = 0.00000;
   ...
   if ( (self.spawnflags & 4.00000) ) {
      ...
      while ( found ) {
         ...
         if ( (vlen ( (found.origin - self.origin)) < 200.00000) ) {
...
local float fire_range = 0.00000;
   ...
   if ( ((self.goalentity.health <= 0.00000) && self.health) ) {
