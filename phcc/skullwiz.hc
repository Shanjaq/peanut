...
// BAER 
// Hacked this to make skull wizards summon smaller varieties than Shadow Wizards.
// also, shadow wizards only summon the brown and red type.
void  ()skullwiz_summon =  
{
	local vector newangle = '0.00000 0.00000 0.00000';
	local vector spot1 = '0.00000 0.00000 0.00000';
	...
	local float loop_cnt = 0.00000;
   	...
   	self.lifetime = (time + 99999.00000); //BAER - Spiders live forever! daaahahahahaha
        if (self.controller.classname == "monster_skull_wizard") // BAER added this
	{
   		self.skin = 1.00000;
		if (random(100) <= self.controller.monsterlevel * 10)
		{
			self.classname = "monster_spider_yellow_large";
			self.health = 75;
			self.experience_value = 100;
			self.max_scale = 1.2;
			self.mass = 3;
			setsize ( self, '-12.00000 -12.00000 0.00000', '12.00000 12.00000 18.00000');
		}
		else
		{
	...
	else
	{
		if (random(100) <= self.controller.monsterlevel * 10)
		{
			self.classname = "monster_spider_red_large";
			self.health = 175;
			self.experience_value = 225;
			self.max_scale = 1.4;
			self.mass = 3;
			setsize ( self, '-14.00000 -14.00000 0.00000', '14.00000 14.00000 21.00000'); 
		}
		else
		{
			self.classname = "monster_spider_red_small";
			self.health = 50;
			self.experience_value = 50;
			self.max_scale = 0.9;
			self.mass = 1;
			setsize ( self, '-9.00000 -9.00000 0.00000', '9.00000 9.00000 14.00000');
		}
	}
   	//self.health = 5.00000;
...
// BAER - GROSS HACK! DAAAHAHAHAHAHAH..
// Umm so i added 3 minimum spawn instead of just one, and added another 15% chancer,
// and added a 50% chancer. so it might summon 6 spiders at most.
   	if ( (self.frame == 35) ) 
	{
      	spider_spawn ( 0);
		spider_spawn ( 1);
...
void  ()SkullMissile_Twist =  
{
   	...
   	AdvanceThinkTime(self,HX_FRAME_TIME * 2); // BAER changed from .2
	//if (random() > 0.66)
...
local float max_scale2 = 0.00000;
   AdvanceThinkTime(self,HX_FRAME_TIME);
   ...
   //}
   if ( (self.scale >= max_scale2) ) {

      self.scale = max_scale2;
...
void  ()skullwiz_melee =  
{
   	...
   	if (self.frame == 78.00000) 
	{
      	if (self.classname == "monster_skull_wizard")
		{
// BAER - skull wizard has 15 percent chance of summoning things.
                        if (random() < 0.15)
			{
				skullwiz_summoninit();
			}
		...
		{
// BAER - Shadow Wizard has 25 percent chance of summoning things.
         		if (random() < 0.25000) 
			{
            		skullwiz_summoninit ( );
         		} 
...
void  ()monster_skull_wizard_lord =  {
   ...
   self.monsterclass = CLASS_LEADER;
	self.money = 300 + rint(random(200));
   self.skin = 1.00000;
   self.scale = 1.20000;
   walkmonster_start ( );
};
