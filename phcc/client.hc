	...
	if (survivor) {
		...
		while ( found ) {
			...
			if ( (found.classname == "player") ) {
				...
				if (player_count == 0) {
					spot_1 = found.best_inning_time;
					name_1 = found.netname;
				}
				if (player_count == 1) {
					spot_2 = found.best_inning_time;
					name_2 = found.netname;
				}
				if (player_count == 2) {
					spot_3 = found.best_inning_time;
					name_3 = found.netname;
				}
				if (player_count == 3) {
					spot_4 = found.best_inning_time;
					name_4 = found.netname;
				}
		...
		//centerprint (self, "you have survived@this long:   ", self.best_inning_time);
		sprint(self, self.netname);
		sprint(self, "\n<<<<<<<<<<#>>>>>>>>>>\n");
		sprint(self, "longest innings in seconds:\n\n");

		sprint(self, name_1);
		sprint(self, "------");
		sprint(self, ftos(spot_1));
		sprint(self, "\n");

		sprint(self, name_2);
		sprint(self, "------");
		sprint(self, ftos(spot_2));
		sprint(self, "\n");

		sprint(self, name_3);
		sprint(self, "------");
		sprint(self, ftos(spot_3));
		sprint(self, "\n");

		sprint(self, name_4);
		sprint(self, "------");
		sprint(self, ftos(spot_4));
	}
};
...
void  ()execute_changelevel =  {
	if (self.tele_dropped == 1) {
		AdvanceThinkTime(self.teledrop_dest,0.01);
		self.teledrop_dest.think = SUB_Remove;
	}

	//shan{
	// AdvanceThinkTime(self.natural, 0.01);
	// self.natural.think = SUB_Remove;
	//shan}
...
void  ()FindDMLevel =  {
	...
	nextmap = string_null;
	if ( ((cvar ( "registered") != 0.00000) || (cvar ( "oem") != 0.00000)) ) {
...
void  ()changelevel_touch =  {
	...
	SUB_UseTargets ( );
	if ( (((cvar ( "registered") == 0.00000) && (cvar ( "oem") == 0.00000)) && (nextmap == "village1")) ) {
	...
	if ( (deathmatch == 0.00000) ) {
		...
		if ((nextmap == "rider1a") || (nextmap == "meso9") || (nextmap == "rider2c") || (nextmap == "romeric6")) {
			other.velocity = '0 0 0';		
			setorigin(other, (other.origin - (v_forward * 100)));
			//		transporter_active = 1;
			other.magic_finished = time + 2.0;
			other.cscale = 1;
			localcmd("save status\n");
			GotoNextMap ( );
			return ;
		} else {
			GotoNextMap ( );
...
entity  ()SelectSpawnPoint =  {
	...
	if (((mapname == "meso2") || (mapname == "egypt1") || (mapname == "romeric1")) && !(deathmatch)) {
		if ((self.experience > 0) && (self.money == 0)) {
			localcmd("load status\n");
		}
	}
...
void  ()PutClientInServer =  {
	...
	if ( self.newclass ) {
		...
		if ( (self.newclass == CLASS_PALADIN) ) {
			...
			if ( (self.newclass == CLASS_CRUSADER) ) {

				bprint ( "Crusader!\n");
			} else {

				if ( (self.newclass == CLASS_NECROMANCER) ) {
	...
	if ( coop ) {
		...
		if ( (self.bluemana < 25.00000) ) {

			self.bluemana = 25.00000;

		}
		if ( (self.greenmana < 25.00000) ) {

			self.greenmana = 25.00000;
...
void  (float TimeDiff)ClientReEnter =  {
	...
	if ( ((!(self.flags2 & FL_ALIVE) || (self.health < 1.00000)) || ((self.newclass && !deathmatch) && !coop)) ) {
	...
	self.glyph_time += TimeDiff;
	self.LfingerC += TimeDiff;
	self.RfingerC += TimeDiff;
	self.attack_finished = 0.00000;
	self.last_attack = 0.00000;
	self.light_level = 128.00000;
	self.dmg = 2.00000;
...
void  ()CheckCrouch =  {
	if ((self.UFO == 1) && (self.flags2 & FL2_CROUCHED)) {
		PlayerCrouching ( );
	} else {
...
void  ()PlayerPreThink =  {
	...
	if ( (self.viewentity != self) ) {
		...
		if ( (self.viewentity.classname != "chasecam") ) {
			...
			CameraViewAngles ( self, self.viewentity, 0, 0, 0);
		} else {
	...
	if ( (self.rings & RING_REGENERATION) ) {
		...
		if ( (self.ring_regen_time < time) ) {
			...
			if ( ((self.ring_regeneration < 10.00000) && !(self.rings_low & RING_REGENERATION)) ) {
				...
				sound ( self, CHAN_BODY, "misc/secret.wav", 1.00000, ATTN_NORM);
...
void  ()PlayerPostThink =  {
	...
	if ( self.deadflag ) {
		if (deathmatch) {
			if (self.inning_time > self.best_inning_time) {
				self.best_inning_time = self.inning_time;
			}
			self.inning_time = 0;
		}
		return ;
	...
	if ( ((((self.jump_flag * (self.mass / 10.00000)) < -300.00000) && (self.flags & FL_ONGROUND)) && (self.health > 0.00000)) ) {
		...
		if ( ((self.scale > 1.00000) && ((self.jump_flag * (self.mass / 10.00000)) < -500.00000)) ) {
			...
			MonsterQuake ( ((self.mass / 10.00000) * self.jump_flag), self);
